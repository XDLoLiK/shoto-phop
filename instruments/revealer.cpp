...
Revealer::Revealer(const std::string& icon, int x, int y):
	Instrument(icon, x, y),
	m_curPoint ({-1, -1}),
	m_prevPoint({-1, -1})
{
...
bool Revealer::mouseLeave(Surface*, booba::Event*)
{
...
bool Revealer::apply(Surface* surface, booba::Event* event)
{
	bool res = false;
...
void Revealer::drawPoint(Surface* surface, const std::pair<int, int>& point)
{
	int width  = 0;
	int height = 0;
...
void Revealer::drawLine(Surface* surface)
{
